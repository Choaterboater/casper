...
export const countWords: ToolDefinition = {
  name: "count_words",
  description: "Count the whitespace-separated words in the input text.",
  inputSchema: {
    type: "object",
  ...
  execute: (args) => String(text(args, "text").split(/\s+/).filter(Boolean).length),
};

/** Catalog order is discovery order. */
export const tools: readonly ToolDefinition[] = [countWords, reverseText];
