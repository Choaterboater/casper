...
```powershell
powershell -ExecutionPolicy Bypass -c "irm https://github.com/Choaterboater/casper/releases/download/v0.1.1/install.ps1 | iex"
```

These commands target the planned **v0.1.1 preview** and will resolve only after it
...
Installer defaults now target `Choaterboater/casper` at tag `v0.1.1`; the repository
...
--version 0.1.1         # require this exact installed version
...
```bash
casper --help
casper --version     # casper 0.1.1 (/absolute/path/to/src/cli.ts or the binary)
```
