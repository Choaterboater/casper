...
```powershell
[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12; irm https://github.com/Choaterboater/casper/releases/download/v0.2.3/install.ps1 | iex
```
...
```sh
curl -fsSL https://github.com/Choaterboater/casper/releases/download/v0.2.3/install.sh | sh
```
...
access. These commands pin **v0.2.3**: re-running reinstalls that preview. For a
newer preview, use its release URL; GitHub's `latest/download` route excludes
prereleases. Useful `install.sh` options: `--dir <path>`, `--version 0.2.3`,
...
casper --version     # casper 0.2.3 (/absolute/path/of/the/binary/or/cli.ts)
```
