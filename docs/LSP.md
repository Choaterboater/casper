...
- Startup consent is cancellable before process creation; immediate disconnect cannot launch a late server. Explicit reconnect only, no background healing. Shutdown attempts `shutdown`/`exit`, then kills the POSIX process group; Windows direct-child behavior is unvalidated. Escaped/daemonized descendants are not guaranteed to be cleaned up.
- No dynamic registration, server-initiated workspace writes, command execution, code actions, formatting, remote transport, or automatic installation in Phase 5.
