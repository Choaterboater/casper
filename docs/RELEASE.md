...
This is an unsigned early-preview distribution. The build and served-directory
shape are verified locally on macOS; Windows/Linux still require host testing.
Installers target the version-specific `v0.1.0` GitHub release, not `latest`.

## Current release blockers

**Only source is published; no binary release or working installer URL exists yet.**

- Compiled image processing currently depends on a Photon WASM file in the build
  tree. A portability probe succeeds with that file present and returns no image
  after it is hidden. Fix resource loading and add a standalone regression before
  release; core CLI and diagram checks do not exercise this path.
- Complete third-party license and notice coverage for bundled dependencies and
  the Bun runtime before distributing binaries. Casper's MIT license does not
  replace their licenses.
- Windows and Linux require real-host testing. Unsigned binaries may trigger OS
  security warnings; cross-compilation is not host validation.

The publishing steps below describe the intended process, not a completed release.
...
The compiled binary embeds Bun and bundled JavaScript. The intended standalone
installation needs neither the checkout nor Bun, but the image-resource blocker
above prevents claiming complete self-containment today. The build fails if
...
- **Requires a successful version probe before replacement.** The staged executable's
  `--version` must exit 0 and produce nonempty output; a requested version must match
  exactly. Matching output with a nonzero exit is still a failure. The success message
  reports that staged probe; neither installer executes the final path again. A rejected
  probe preserves the previous installation.
