...
`v0.1.3` preview. Nothing is published, and no Windows host has run `install.ps1`.**
...
1. Choose the release host and set the default `CASPER_BASE_URL` in both
   ...
   `https://github.com/Choaterboater/casper/releases/download/v0.1.3`.
...
3. Upload **every file** in `dist/release/` to one directory of a release host
   (for this preview, the GitHub Release tagged `v0.1.3`). The
...
then create `v0.1.3` against that exact commit as a GitHub prerelease. Upload binaries
...
- **No npm or Homebrew channel.** `npm i -g casper` is not available: the name `casper`
  is taken on npm (v0.1.3), and a Bun-native CLI would still require Bun on the target
...
- **Nothing is published.** `dist/release/` is built and locally verified; installer
  defaults target `https://github.com/Choaterboater/casper/releases/download/v0.1.3`.
