...
def exercise(bun, repo, root, no_color=False):
    ...
    try:
        ...
        s.send("Q")
        s.pump(0.1)
        assert "x" * 93 + "Qxx" in s.screen.text().replace("\n", ""), s.screen.text()
        # The exclusive handoff must retain readline history as well as its draft.
        s.send("\x01\x0b\x1b[A")
        s.pump(0.1)
        assert s.screen.text().rstrip().endswith("> /model"), s.screen.text()
        ...
        assert saved == {"defaultProvider": "fixture", "defaultModel": "second"}, saved
