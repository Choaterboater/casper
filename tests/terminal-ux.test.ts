...
import { formatToolActivity, redactPreview, terminalText } from "../src/tui/format";
import { posixOnly } from "./support/platform";

const roots: string[] = [];
afterEach(async () => { await Promise.all(roots.splice(0).map((root) => rm(root, { recursive: true, force: true }))); });

test("tool previews retain targets and errors while redacting common credentials", () => {
  ...
  expect(failure).toContain("src/missing.ts"); expect(failure).toContain("ENOENT");
  expect(failure).not.toContain("secret");
  expect(redactPreview("sk-abcdefghijk ghp_abcdefghijk")).toBe("<redacted> <redacted>");
});

test("terminal output neutralizes cursor, title and bidi controls", () => {
